...
class DiscordClient(discord.Client):
    ...
    async def on_ready(self):
        logger.info('Logged into Discord as ' + self.user.name + ' (' + str(self.user.id) + ')')
        ...
        for server in self.guilds:
            ...
            for channel in server.channels:
                if channel.__class__.__name__ == "TextChannel":
                    ...
                    logger.error("channel type not handled: " + channel.__class__.__name__)
    ...
    async def on_message(self, message):
        ...
        if message.attachments:
            logger.debug('message attachments: ' + str(message.attachments))
        if message.embeds:
            import json
            logger.debug('message embeds: ' + str([str(x.to_dict()) for x in message.embeds]))
